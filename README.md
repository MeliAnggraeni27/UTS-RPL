...
Meli Anggreini1, Muhamad Raihan Rabbani2
...
Kata kunci : Sistem Absensi, Manajemen Sumber Daya.

Abstract
This study aims to design and implement a web-based attendance system as a modern solution to replace manual presence recording processes which are often prone to errors, data manipulation, and time inefficiency. Traditional methods frequently yield inaccurate data, slow down the recapitulation process, and hinder real-time attendance monitoring by management. The proposed web-based attendance system is designed using the Model-View-Controller (MVC) architecture, utilizing technologies such as PHP, MySQL, and a responsive framework to ensure accessibility across various devices. The system's main features include self-attendance recording, location verification using Geolocation, integration with existing academic/employee databases, and automated reporting. Implementation results indicate that the system successfully enhances the accuracy of attendance records up to 99% and significantly reduces the time required for monthly attendance data recapitulation. Consequently, the web-based attendance system prove.
Keyword : Web-Based Attendance System, Human Resource
...
1.	Metode penelitian
Metodologi yang diterapkan dalam penelitian ini adalah Metode Waterfall, yang dicirikan oleh alur kerja sistematis dan berurutan. Dalam konteks ini, pengembangan sistem dipecah menjadi beberapa fase yang saling terkait, di mana transisi ke fase berikutnya hanya diperbolehkan setelah seluruh pekerjaan pada fase sebelumnya rampung dan disetujui.
 
Setiap fase menghasilkan dokumen lengkap (output) yang menjadi input untuk fase berikutnya. Dokumentasi yang lengkap dan detail adalah kunci dalam model ini.
...
a.	Use Case Diagram
 ![alt text](https://github.com/MeliAnggraeni27/UTS-RPL/blob/main/use%20case%20rpl.png?raw=true)
...
o	Setiap validasi memiliki kondisi keluar (exit condition) yang jelas (Ya/Tidak) yang mengarah ke langkah berikutnya atau ke kondisi akhir yang gagal. Hal ini memudahkan debugging (pencarian kesalahan); jika absensi gagal, developer dapat dengan cepat mengetahui di tahap validasi mana kegagalan terjadi.
Reusability (Kemampuan Pakai Ulang): Baik
...
o	Modul Login: Bertanggung jawab penuh atas validasi username dan password.
o	Modul Validasi Waktu: Bertanggung jawab penuh atas pengecekan rentang waktu absensi.
o	Modul Validasi Lokasi: Bertanggung jawab penuh atas pengecekan koordinat GPS/lokasi. Hal ini memungkinkan setiap "modul" tersebut dapat dipanggil atau digunakan kembali (reusable) di bagian sistem lain yang membutuhkan validasi serupa (misalnya, validasi waktu/lokasi untuk check-in atau check-out guru).
...
o	Prosedur penyimpanan ke database dapat dijadikan service terpisah yang dapat digunakan oleh berbagai proses pengiriman data lain dalam sistem (misalnya, pengiriman tugas, pengiriman formulir).

Saran Improvement & Fitur Baru
1. Fitur Absensi dengan QR Code
•	Guru menghasilkan QR code unik untuk setiap kelas.
•	Siswa cukup scan QR untuk absen.
•	Mencegah titip absen.
•	Cepat dan efisien.
Manfaat: Proses absensi lebih cepat dan akurat
2. Validasi Lokasi (GPS / Geolocation)
•	Sistem mengecek apakah siswa berada dalam radius sekolah.
•	Jika siswa tidak di area sekolah → absensi ditolak.
Manfaat: Menghindari absensi dari rumah atau tempat lain
3. Deteksi Wajah (Face Recognition)
•	Absensi menggunakan kamera HP/laptop.
•	Sistem mencocokkan dengan foto siswa di database.
Manfaat: Anti titip absen, sangat akurat
4. Notifikasi Otomatis ke Orang Tua
•	Sistem mengirimkan WhatsApp atau email:
o	“Anak Anda hadir pukul 07:05.”
o	“Anak Anda belum absen sampai pukul 07:30.”
Manfaat: Orang tua bisa memantau disiplin anak.
5. Rekap Otomatis per Bulan
•	Sistem membuat laporan otomatis:
o	Hadir
o	Izin
o	Sakit
o	Alfa
Manfaat: Guru tidak perlu menghitung manual.

6. Dashboard Statistik
Menampilkan:
•	grafik jumlah kehadiran
•	persentase ketidakhadiran
•	peringkat kedisiplinan siswa
Manfaat: Visualisasi memudahkan evaluasi.
7. Sistem Izin & Sakit Online
Siswa bisa:
•	mengajukan izin
•	upload bukti foto (misal surat dokter)
•	menunggu approval guru
Manfaat: Administrasi izin jadi rapi dan terdokumentasi.
8. Integrasi Kalender Sekolah
•	Absensi otomatis menyesuaikan hari libur.
•	Ada notifikasi jadwal ujian atau kegiatan.
9. Mode Offline (Sinkronisasi Ketika Online)
Untuk sekolah yang internetnya terbatas:
•	absensi masih bisa dilakukan offline
•	data akan tersinkron ketika koneksi kembali
10. Log Aktivitas (Activity Log)
Mencatat:
•	siapa login
•	kapan absen
•	perubahan data apa saja
Manfaat: keamanan data meningkat.
